...
The generator architecture itself follows an autoencoder structure. "An autoencoder[...] is trained to attempt to copy its input to its output."[^4]. Autoencoders use an encoder-decoder structure to first reduce a high dimensional input to an internal representation of the input. Using this approach essential parts of the input could be extracted. The following decoder part is used to reconstruct the input from the internal representation as close as possible. In our case, we are using 9 residual blocks between the encoder-decoder part of the generator[^5]. The basic idea of these residual blocks is to add a skip connection, which enables the block to skip the activation of a layer and add up this activation on a later layer[^5]. Using this approach certain layers could be skipped.
